...
function PLAYER:BeginSpectate()
	...
	if PS then self:PS_PlayerDeath() end
end
...
function PLAYER:ShouldStaySpectating() -- check if he should respawn
	...
	if self.StaySpectating == nil then self.StaySpectating = false end
...
function PLAYER:ChangeSpectate()
	...
	if self.ObsMode2 > 2 then self.ObsMode2 = 0 end
	...
	if self.ObsMode2 > 0 then
		...
		for k,self in ipairs(player.GetAll()) do
			if self:Alive() and not self:GetSpectate() then table.insert(pool,self) end
		end
...
function PLAYER:SpecModify(n)
	...
	if self.SpecEntIdx > #pool then self.SpecEntIdx = 1 end
	if self.SpecEntIdx < 1 then self.SpecEntIdx = #pool end
	if #pool > 0 then
	...
	if self:GetObserverMode() ~= OBS_MODE_IN_EYE then self:SetupHands(nil) end
end
...
hook.Add("KeyPress","DeathrunSpectateChangeObserverMode",function(self,key)
	if self:GetSpectate() then
		if key == IN_JUMP then self:ChangeSpectate() end
