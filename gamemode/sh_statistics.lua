...
if SERVER then
	...
	findendmap()
	hook.Add("InitPostEntity","DeathrunFindEndZone",function() findendmap() end)
	hook.Add("DeathrunBeginPrep","DeathrunSendRecords",function()
		...
		if endmap ~= nil and res ~= false then
			...
			net.WriteVector(.5 * (endmap.pos1 + endmap.pos2))
		...
		for k,ply in ipairs(player.GetAll()) do
			...
			if endmap ~= nil and res2 ~= false then
				local seconds = -1
				if res2 ~= nil then if res2[1] then if res2[1]["seconds"] then seconds = res2[1]["seconds"] end end end
	...
	function DR.SteamToNick(sid)
		...
		if res then nick = Base64Decode(res[1]["nick"]) end
		return nick
	end
	...
	hook.Add("PlayerAuthed","CreateStatsRow",function(ply,steamid,uid)
		...
		if not res then res = sql.Query("INSERT INTO deathrun_stats VALUES ( '" .. steamid .. "', 0, 0, 0, 0 )") end
...
if CLIENT then
	...
	net.Receive("deathrun_send_map_pb",function() DR.MapPBCache = net.ReadFloat() end)
	DR.PlayerStatsCache = {}
	net.Receive("deathrun_send_stats",function()
		...
		for k,v in ipairs(player.GetAll()) do
			if name == v:SteamID() then name = v:Nick() end
		end
	...
	hook.Add("PostDrawTranslucentRenderables","statsdisplay",function()
		...
		if t < lifetime then
			h = 80 + 75 * #labels
			cam.Start3D2D(stats3d.pos,stats3d.ang,.04)
			render.ClearStencil()
			...
			for i = 1,#labels do
				DR.ShadowTextSimple(labels[i],"Deathrun_3D2D_Small",x + 20,y + 100 + 70 * (i - 1),DR.Colors.Grey3,TEXT_ALIGN_LEFT,TEXT_ALIGN_TOP,0)
			end

			for i = 1,#stats3d.data do
				DR.ShadowTextSimple(tostring(stats3d.data[i]),"Deathrun_3D2D_Small",x + w - 20,y + 100 + 70 * (i - 1),DR.Colors.Turq,TEXT_ALIGN_RIGHT,TEXT_ALIGN_TOP,0)
			end
		...
		if DR.MapRecordsDrawPos ~= Vector(0,0,0) and DR.MapRecordsDrawPos ~= nil then
			...
			if dist < 1000 then
				...
				--local scale = math.Clamp( InverseLerp( dist, 1000, 400 ), 0,1) * .12
				--if dist < 20 then
				...
				if DR.MapRecordsCache[1] ~= nil then
					for i = 1,#DR.MapRecordsCache + 2 do
						...
						if i <= #DR.MapRecordsCache then
							...
							DR.ShadowTextSimple(tostring(i) .. ". " .. string.sub(v["nickname"] or "",1,24),"Deathrun_3D2D_Large",-700,-150 + 100 * k,DR.Colors.Clouds,TEXT_ALIGN_LEFT,TEXT_ALIGN_TOP,2)
							DR.ShadowTextSimple(string.ToMinutesSecondsMilliseconds(v["seconds"] or "0"),"Deathrun_3D2D_Large",700,-150 + 100 * k,DR.Colors.Turq,TEXT_ALIGN_RIGHT,TEXT_ALIGN_TOP,2)
							surface.SetDrawColor(DR.Colors.Turq)
							surface.DrawRect(-700,-150 + 100 * k + 80,1400,2)
						elseif i == #DR.MapRecordsCache + 2 and DR.MapPBCache ~= 0 then
							DR.ShadowTextSimple("Personal Best","Deathrun_3D2D_Large",-700,-150 + 100 * k,DR.Colors.Clouds,TEXT_ALIGN_LEFT,TEXT_ALIGN_TOP,2)
							DR.ShadowTextSimple(string.ToMinutesSecondsMilliseconds(DR.MapPBCache or 0),"Deathrun_3D2D_Large",700,-150 + 100 * k,DR.Colors.Turq,TEXT_ALIGN_RIGHT,TEXT_ALIGN_TOP,2)
							surface.SetDrawColor(DR.Colors.Turq)
							surface.DrawRect(-700,-150 + 100 * k + 80,1400,2)
						end
					end
				else
					DR.ShadowTextSimple("No records yet!","Deathrun_3D2D_Large",0,-200,DR.Colors.Clouds,TEXT_ALIGN_CENTER,TEXT_ALIGN_TOP,2)
				end
...
-- { [ ( 1 - .5^(death_wins + runner_wins) ) / .5 ] -1 } * sqrt(KDR)
-- where KDR = K/D
if SERVER then end
