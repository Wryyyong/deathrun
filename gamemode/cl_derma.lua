...
function BUTTN:Init()
	...
	self.b.OnCursorEntered = function() self.hover = true end
	self.b.OnCursorExited = function() self.hover = false end
	self.b.OnMousePressed = function(self2,mkey) self:OnMousePressed(mkey) end
	function self.b:Paint()
	end
...
function BUTTN:OnMousePressed(mkey)
	if not self.disabled then
		if mkey == MOUSE_LEFT then self:DoClick() end
		if mkey == MOUSE_RIGHT then self:DoRightClick() end
	end
end

function BUTTN:IsDown()
	if self.hover == true then if input.IsMouseDown(MOUSE_LEFT) then return true end end
	return false
end
...
function MPANEL:Init()
	...
	self.buttons = {}
	self.tabs = {}
	self.color = {HexColor("#c0392b"),HexColor("#e74c3c")}
	self:PerformLayout()
	self.activetab = 0
...
function MPANEL:PerformLayout()
	...
	if self.navright then self.navright:SetPos(self:GetWide() - 24) end
	if self.spacer then self.spacer:SetSize(self:GetWide(),4) end
	for i = 1,#self.tabs do
...
concommand.Add("deathrun_test_derma",function() vgui.Create("deathrun_window") end)
...
function AUTOGGLE:Toggle()
	...
	if self.convar then RunConsoleCommand(self.convar:GetName(),tostring(self.state == true and 1 or 0)) end
