...
function DR.OpenCrosshairCreator()
	...
	drawx:SetSize(panel:GetWide() * .5 - 2,panel:GetTall())
	drawx:SetPos(0,0)
	function drawx:Paint(w,h)
		...
		surface.DrawRect(w * .5 - thick * .5,h * .5 - (size + gap * .5),thick,size)
		surface.DrawRect(w * .5 - thick * .5,h * .5 + gap * .5,thick,size)
		surface.DrawRect(w * .5 + gap * .5,h * .5 - thick * .5,size,thick)
		surface.DrawRect(w * .5 - (size + gap * .5),h * .5 - thick * .5,size,thick)
	end

	local controls = vgui.Create("DPanel",panel)
	controls:SetSize(panel:GetWide() * .5 - 2,panel:GetTall())
	...
	for k,v in pairs(crosshair_convars) do
		...
		elseif ty == "boolean" then
			...
			lbl:SetFont("Deathrun_Derma_ExtraSmall")
...
function DR.OpenZoneEditor()
	...
	lbl:SizeToContents()
	lbl:SetWide(dlist:GetWide() * .5 - 2)
	dlist:Add(lbl)
	local te = vgui.Create("DTextEntry")
	te:SetSize(dlist:GetWide() * .5 - 2,18)
	te:SetText("new_zone")
	dlist:Add(te)
	local lbl = vgui.Create("DLabel")
	lbl:SetFont("Deathrun_Derma_ExtraSmall")
	...
	dd:SetSize(dlist:GetWide() * .5 - 2,18)
	dd:SetValue("end")
...
function DR.GetWordWrapText(text,w,font)
	...
	for i = 1,#args do
		...
		if i == #args then displaytext = displaytext .. displayline end
	end
	return displaytext
end
