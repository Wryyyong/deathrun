...
function MV:NewDermaRow(tbl_cols,w,h,customColor,customColor2,doclick)
	if doclick == nil then doclick = function() end end
	...
	btn.DoClick = doclick or function(self) self:GetParent():DoClick() end
	return panel
end
...
function MV:RepopulateMapList()
	if IsValid(MV.AllMapsListList) then
		...
		local lb = dlist:Add("DLabel")
		lb:SetFont("Deathrun_Derma_ExtraSmall")
...
timer.Create("MapvoteCountdownTimer",.2,0,function()
	if MV.Active == true then
		...
		if IsValid(MV.VotingPanelDerma) then
			...
			if MV.TimeLeft <= 0 then timer.Simple(4,function() if IsValid(MV.VotingPanelDerma) then MV.VotingPanelDerma:Close() end end) end
		end

		if MV.TimeLeft < 0 then MV.TimeLeft = 0 end
	end
end)
...
hook.Add("SetupMove","MapvoteReceiveKeys",function()
	if not vgui.CursorVisible() and MV.Active then
		for i = 1,#keynums do
			...
			if input.WasKeyPressed(keynums[i]) then RunConsoleCommand("mapvote_vote",mapname) end
		end
	end
end)
