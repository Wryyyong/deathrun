...
if SERVER then
	function DR:RewardPlayer(ply,amt,reason)
		...
		if hasPointshop then
			...
			if PointshopRewardMessage:GetBool() then ply:PS_Notify("You were given " .. tostring(amt) .. " points for " .. (reason or "playing") .. "!") end
		end
		...
		if hasRedactedHub then
			...
			if amt <= storemoney then
				...
				if PointshopRewardMessage:GetBool() then ply:DeathrunChatPrint("You were given " .. tostring(amt) .. " points for " .. (reason or "playing") .. "!") end
			else
				if PointshopRewardMessage:GetBool() then ply:DeathrunChatPrint("Unfortunately the store does not have enough points to reward you.") end
			end
		end
	...
	hook.Add("DeathrunPlayerFinishMap","PointshopRewards",function(ply,zname,z,place) DR:RewardPlayer(ply,PointshopFinishReward:GetInt(),"finishing the map") end)
	hook.Add("PlayerDeath","PointshopRewards",function(ply,inflictor,attacker) if attacker:IsPlayer() then if ply:Team() ~= attacker:Team() then DR:RewardPlayer(attacker,PointshopKillReward:GetInt(),"killing " .. ply:Nick()) end end end)
	hook.Add("DeathrunRoundWin","PointshopRewards",function(winner)
		for k,v in ipairs(player.GetAllPlaying()) do
			if v:Team() == winner then DR:RewardPlayer(v,PointshopWinReward:GetInt(),"winning the round") end
		end
	end)
end
