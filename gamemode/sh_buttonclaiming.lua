...
if SERVER then
		...
		for k,v in ipairs(but_ents) do
			if not buttons[v:MapCreationID()] then
				...
				for _,ply in ipairs(team.GetPlayers(TEAM_DEATH)) do
					if ply:SteamID() == buttons[v:MapCreationID()].claimedPlayer then currentClaimPly = ply end
				end

				local currentClaimPlyDist = 99999
				if currentClaimPly then currentClaimPlyDist = currentClaimPly:EyePos():Distance(pos) end
	...
	timer.Create("CheckButtonClaims",.35,0,function() CheckButtonClaims() end)
	local function PlayerCanButton(ply,ent)
		if not ply:Alive() or ply:Team() == TEAM_SPECTATOR or ply:GetObserverMode() ~= OBS_MODE_NONE then return false end
...
if CLIENT then
	net.Receive("UpdateButtonClaims",function() buttons = net.ReadTable() end)
	hook.Add("HUDPaint","DeathrunButtonClaimHUD",function()
		...
		for k,v in pairs(buttons) do
			...
			if dist < claim_radius * 3 then
				local alpha = 255
				if dist > claim_radius then alpha = Lerp(DR.InverseLerp(dist,claim_radius,claim_radius * 3),255,0) end
