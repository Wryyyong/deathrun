...
function player.GetAllPlaying()
	...
	for k,ply in ipairs(player.GetAll()) do
		if ply then if ply:ShouldStaySpectating() == false then table.insert(pool,ply) end end
	end
	return pool
end
...
local function AutoHop(ply,data)
	if CLIENT then LocalPlayer().AutoJumpEnabled = intToBool(GetConVar("deathrun_autojump"):GetInt()) end
	if lp and ply ~= lp() then return end
	...
	if ba(ButtonData,IN_JUMP) > 0 then if ply:WaterLevel() < 2 and ply:GetMoveType() ~= MOVETYPE_LADDER and not ply:IsOnGround() then data:SetButtons(ba(ButtonData,bn(IN_JUMP))) end end
end
...
function DR.GetAccessLevel(ply)
	...
	if DR.PlayerAccess[id] then access = DR.PlayerAccess[id] end
	if DR.PlayerAccess[id64] then access = DR.PlayerAccess[id64] end
	return access or 1
end
